...
// Subjects are the things one metrics rule is measured over: the files, for a metric about a file, and the
// classes, for a metric about a class.
//
// Both populations are carried because a rule's scope is written before its metric is chosen — `metrics, in
// folder "internal/**", for classes matching "*Service"` narrows both at once — and a metric reads the one
// it is about. That is what keeps the choice out of every caller: nothing between the scope and the number
// has to branch on what kind of metric was picked.
...
// SelectSubjects returns what a metrics rule measures, given the files its scope selected and read, and
// the selectors it wrote about classes.
//
...
func SelectSubjects(files []metricsextraction.FileInfo, classSelectors ...matching.Filter) Subjects {
	...
	if len(classSelectors) == 0 {
		return Subjects{Files: slices.Clone(files), Classes: classes}
	}
	...
	return Subjects{Files: measured, Classes: classes}
}
